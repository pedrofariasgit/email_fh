...
cursor_pg.execute("SELECT COALESCE(MAX(idpessoa), 0) FROM cliente")
...
cursor_sql.execute("""
    ...
    FROM 
        cad_Pessoa p
    INNER JOIN 
        cad_Cliente c ON c.IdPessoa = p.IdPessoa
    WHERE 
        p.Ativo = 1
        AND c.Cliente = 1
        AND p.IdPessoa > ?
...
dados_para_inserir = [(int(row.idpessoa), row.nome) for row in novos_dados]

# 5. Executar insert em lote
if dados_para_inserir:
    execute_batch(cursor_pg, """
        INSERT INTO cliente (idpessoa, nome)
        VALUES (%s, %s)
        ON CONFLICT (idpessoa) DO NOTHING;
    """, dados_para_inserir)
    POSTGRES_CONN.commit()
    print(f"Inseridos {len(dados_para_inserir)} novos registros na tabela 'cliente'.")
else:
